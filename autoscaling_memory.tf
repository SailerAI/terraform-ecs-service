resource "aws_appautoscaling_policy" "memory_high" {
  count = var.scale_type == "memory" ? 1 : 0

  resource_id        = aws_appautoscaling_target.main.resource_id
  service_namespace  = aws_appautoscaling_target.main.service_namespace
  scalable_dimension = aws_appautoscaling_target.main.scalable_dimension

  name = format("%s-%s-memory-scale-out", var.cluster_name, var.service_name)
...
resource "aws_cloudwatch_metric_alarm" "memory_high" {
  ...
  alarm_name = format("%s-%s-memory-scale-out", var.cluster_name, var.service_name)
  ...
  alarm_actions = [
    aws_appautoscaling_policy.memory_high[count.index].arn
  ]
}

resource "aws_appautoscaling_policy" "memory_low" {
  count = var.autoscaling_enabled && var.scale_type == "memory" ? 1 : 0

  resource_id        = aws_appautoscaling_target.main.resource_id
  service_namespace  = aws_appautoscaling_target.main.service_namespace
  scalable_dimension = aws_appautoscaling_target.main.scalable_dimension

  name = format("%s-%s-memory-scale-in", var.cluster_name, var.service_name)
  ...
  step_scaling_policy_configuration {
    ...
    step_adjustment {
      metric_interval_lower_bound = 0
      metric_interval_upper_bound = var.scale_in_memory_threshold
      scaling_adjustment          = var.scale_in_adjustment
    }

    step_adjustment {
      metric_interval_lower_bound = var.scale_in_memory_threshold
...
resource "aws_cloudwatch_metric_alarm" "memory_low" {
  ...
  alarm_name = format("%s-%s-memory-scale-in", var.cluster_name, var.service_name)
  ...
  alarm_actions = [
    aws_appautoscaling_policy.memory_low[count.index].arn
  ]
}